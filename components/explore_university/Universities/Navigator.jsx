...
import Course from '@/components/explore_university/Universities/Course.jsx';
const Navigator = ({ props }) => {
  ...
  useEffect(() => {
    ...
    const loadCourses = async () => {
      try {
        const slugs = univdata.map((item) => item.slug).filter(Boolean);
        if (!slugs.length) {
          setCoursesList([]);
          return;
        }
  ...
  return (
    <>
      ...
      {univdata &&
        univdata.map((undata, index) => {
          return (
            <React.Fragment key={undata?.id || undata?.slug || index}>
              <div
                ref={about}

...
import ExploreMainPage from '@/components/explore_university/ExploreMainPage.jsx';
import { useAuth } from '@/context/AuthProvider.jsx';
import { useRouter } from 'next/navigation';
import { getRouteState, ROUTE_STATE_KEYS } from '@/lib/routeState.js';
...
const ExploreUniversityData = () => {
  ...
  useEffect(() => {
    ...
    if (payload?.udata) {
      const normalized = Array.isArray(payload.udata)
        ? payload.udata
        : [payload.udata];
      deferStateUpdate(() => {
        setunData(normalized);
        setImg(payload.url || '');
      });
    } else {
      router.replace('/explore');
    }
  }, [router, deferStateUpdate]);
  const navigateLogin = () => {
    if (currentUser) router.push('/dashboard/applications');
    else
  ...
  return (
    <>
      <div className="max-w-5xl mx-auto bg-transparent px-5 p-10 rounded my-5">
        {undata &&
          undata.map((data, index) => {
            return (
              <div
                key={data?.id || data?.UniversityId || index}
              ...
              >
                ...
                <div className="col-span-2 sm:col-span-3 self-center">
                  <div className="flex flex-col flex-wrap">
                    <label className="text font-bold text-1.9lr">
                      {data.Name}
                    </label>

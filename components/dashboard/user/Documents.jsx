...
const Documents = () => {
  const { profileData, uploadDocument, currentUser, handleDocumentDelete } =
    useAuth();
  ...
  const handleDocsChange = (e) => {
    ...
    if (file) {
      ...
      if (sizeInMB > 5) {
        toast.warning('File size should be less than 5 MB', {
          autoClose: 1500,
        });
        return;
      } else {
  ...
  const handelDocumentSubmit = (e) => {
    e.preventDefault();
    const alreadyExists = profileData?.documents?.some(
      (doc) => doc.title === select,
    );
    if (alreadyExists) {
      toast.warning(
        'Document already exists. Please delete the existing one first.',
        {
          autoClose: 1500,
        },
      );
  ...
  return (
    <div className="p-6 sm:p-10 my-10 rounded-md mb-20 bg-white shadow-md">
          ...
          <tbody>
            {profileData?.documents?.map((doc, index) => (
              ...
              >
                ...
                <td className="p-2">
                  <button
                    onClick={() => handleDocumentDelete(doc.id, 'documents')}
                  >
                    <Delete
                      style={{
                        color: 'red',
      ...
      <p className="text-sm text-gray-600 max-w-5xl mx-auto mb-4">
        <strong>Note:</strong> Please upload a clear scanned copy of your
        original documents. Accepted formats are PDF, DOC, DOCX, PNG, JPEG, JPG
        only. Max file size: 5MB.
      </p>

      <form
        onSubmit={handelDocumentSubmit}
        className="max-w-5xl mx-auto space-y-6 bg-gray-50 p-6 rounded-md"
      >
        ...
        <div className="flex justify-start gap-4 pt-4">
          <button
            type="submit"
            className={`px-6 py-2 rounded-md text-sm border-2 ${loading ? 'bg-main text-white border-main hover:bg-blue-700' : 'bg-gray-200 text-gray-600 border-gray-300 cursor-not-allowed'}`}
            disabled={!loading}
          >
            Upload
          </button>

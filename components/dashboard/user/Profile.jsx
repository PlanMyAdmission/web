...
import Education from '@/components/dashboard/user/profile/Education.jsx';
import Experience from '@/components/dashboard/user/profile/Experience.jsx';
import Test_Scores from '@/components/dashboard/user/profile/Test_Scores.jsx';
import About from '@/components/dashboard/user/profile/About.jsx';
...
const Profile = () => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && window.location.hash) {
      ...
      deferStateUpdate(() => setShow(nextHash));
    }
  }, [deferStateUpdate]);
  useEffect(() => {
    const nextPhoto =
    ...
    deferStateUpdate(() => setPhotoURL(nextPhoto));
  }, [currentUser?.photoURL, deferStateUpdate]);
    ...
    <div className="max-w-7xl mx-auto px-4 md:px-0">
      ...
      <div className="flex mt-8 overflow-x-auto hide-scroll-bar border-b border-main">
        {TABS.map((tab) => (
          <button
            key={tab.id}
            className={`py-2 px-4 text-sm md:text-base font-medium transition-colors duration-300 border-b-2 ${show === tab.id ? 'border-main text-main' : 'border-transparent text-gray-500 hover:text-main'}`}
            onClick={() => {
              deferStateUpdate(() => setShow(tab.id));

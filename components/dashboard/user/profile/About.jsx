...
const About = () => {
  ...
  const [loc, setLoc] = useReducer((_, payload) => payload, []);
  ...
  const [AboutData, setAboutData] = useReducer(
    (_, payload) => payload,
    initialAboutState,
  );

  const deferUpdate = (cb) => {
  ...
  const handleChange = (e) => {
    ...
    setLoading(isFormComplete(updated));
  };
  ...
  useEffect(() => {
    if (profileData) {
      ...
      deferUpdate(() => setLoading(isFormComplete(nextData)));
    }
  ...
  const cityOptions = useMemo(() => {
    if (!AboutData.state || stateOptions.length === 0) {
      return [];
    }
    const foundState = stateOptions.find(
      (item) => item.name === AboutData.state,
    );
    return foundState?.cities || [];
  }, [AboutData.state, stateOptions]);
  const fields = [
        ...
        <div className="overflow-hidden max-w-5xl md:mx-auto mx-2">
            ...
            <div className="grid grid-cols-6 gap-6">
              {fields.map(([name, label, disabled, type = 'text']) => (
                ...
                >
                  ...
                  >
                    {label}{' '}
                    {disabled ? '' : <span className="text-main">*</span>}
                  </label>
                  {type === 'textarea' ? (
                    <textarea
                      name={name}
          ...
          <div className="px-4 py-3 text-right sm:px-6">
            <button
              type="submit"
              disabled={!loading}
              className={`inline-flex justify-center rounded-md py-2 px-4 text-sm font-medium focus:outline-none ${loading ? 'bg-main text-white hover:bg-white hover:outline outline-main hover:text-main transition duration-200' : 'bg-light text-black'}`}
            >
              Save
            </button>
          </div>
